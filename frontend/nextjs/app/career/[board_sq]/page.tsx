...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { data: board, confirmedMissing } = await getCommunityBoardDetail('career', board_sq)
...
export default async function CareerDetailPage({ params }: Props) {
  ...
  const { data: board } = await getCommunityBoardDetail('career', board_sq)
  return (
    <>
      {board && (
        <>
          <JsonLd data={discussionForumPostingJsonLd(board, `/career/${board_sq}`)} />
          <JsonLd
            data={breadcrumbJsonLd([
              { name: '홈', path: '/' },
              { name: '커리어소통', path: '/career' },
              { name: board.ttl, path: `/career/${board_sq}` },
            ])}
          />
        </>
      )}
      <Suspense>
        <QnaDetailClient boardSq={board_sq} boardType="career" initialData={board} />
      </Suspense>
    </>
  )
}
