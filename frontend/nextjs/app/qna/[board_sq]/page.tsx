...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { data: qna, confirmedMissing } = await getQnaDetail(board_sq)
  // 확실히 삭제/존재하지 않는 글만 색인 제외 — 일시적 조회 실패는 noindex 하지 않는다
  if (!qna) return { title: 'QnA 상세', robots: { index: confirmedMissing ? false : undefined } }
...
export default async function QnaDetailPage({ params }: Props) {
  ...
  const { data: qna } = await getQnaDetail(board_sq)
  return (
    <>
      {qna && (
        <>
