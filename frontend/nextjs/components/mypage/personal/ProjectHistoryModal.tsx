...
export default function ProjectHistoryModal({ open, allSkills, roleTypes, taskTypes, onClose, onComplete }: Props) {
  ...
  const { validate, fieldProps, bindRef, isInvalid, clearField, clearAll } =
    useFormErrors<keyof typeof EMPTY | 'skills'>()
  const set = (k: keyof typeof EMPTY, v: string | number) => { clearField(k); setForm((p) => ({ ...p, [k]: v })) }
  ...
  const toggle = (s: SkillTag) => {
    clearField('skills')
    setSelectedSkills((prev) => prev.some((x) => x.skillTagSq === s.skillTagSq) ? prev.filter((x) => x.skillTagSq !== s.skillTagSq) : [...prev, s])
  }
  const isSel = (s: SkillTag) => selectedSkills.some((x) => x.skillTagSq === s.skillTagSq)

  const reset = () => { setForm(EMPTY); setSelectedSkills([]); clearAll() }
  const close = () => { reset(); onClose() }

  const submit = () => {
    if (!validate([
      { key: 'name', invalid: !form.name.trim(), message: '프로젝트명을 입력해주세요.' },
      { key: 'startDate', invalid: !form.startDate, message: '프로젝트 근무 기간을 선택하세요.' },
      { key: 'client', invalid: !form.client.trim(), message: '고객사를 입력하세요.' },
      { key: 'workUnit', invalid: form.workUnit === '', message: '업무단을 선택하세요.' },
      { key: 'role', invalid: form.role === '', message: '프로젝트 담당 역할을 선택하세요.' },
      { key: 'skills', invalid: selectedSkills.length === 0, message: '개발환경을 선택하세요.' },
    ])) return
    onComplete({
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="space-y-1"><label className="text-sm">프로젝트명</label><Input {...fieldProps('name')} value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="예: 금융 시스템 구축" /></div>
              <div className="space-y-1">
                <label className="text-sm">참여기간</label>
                <div className="flex flex-wrap items-center gap-1">
                  <Input {...fieldProps('startDate')} type="date" max={today()} className="min-w-0 flex-1" value={form.startDate} onChange={(e) => set('startDate', e.target.value)} />
                  <span>~</span>
                  <Input type="date" max={today()} className="min-w-0 flex-1" value={form.endDate} onChange={(e) => set('endDate', e.target.value)} />
                </div>
              </div>
              <div className="space-y-1"><label className="text-sm">고객사</label><Input {...fieldProps('client')} value={form.client} onChange={(e) => set('client', e.target.value)} placeholder="예: OO은행" /></div>
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <label className="text-sm">업무단</label>
                  <select ref={bindRef('workUnit')} aria-invalid={isInvalid('workUnit') || undefined} value={form.workUnit} onChange={(e) => set('workUnit', e.target.value ? Number(e.target.value) : '')} className="h-9 w-full rounded-md border border-input px-2 text-sm cursor-pointer aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20">
                ...
                <div className="space-y-1">
                  ...
                  <select ref={bindRef('role')} aria-invalid={isInvalid('role') || undefined} value={form.role} onChange={(e) => set('role', e.target.value ? Number(e.target.value) : '')} className="h-9 w-full rounded-md border border-input px-2 text-sm cursor-pointer aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20">
                    <option value="">선택</option>
          ...
          <InvalidFrame ref={bindRef('skills')} invalid={isInvalid('skills')} className="space-y-2">
