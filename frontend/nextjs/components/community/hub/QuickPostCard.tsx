...
export default function QuickPostCard() {
  ...
  return (
    <div className="mb-4 flex items-center gap-2 rounded-lg border bg-muted/30 p-4">
      ...
      <p className="min-w-0 flex-1 truncate text-sm text-muted-foreground">지금 커뮤니티에 글을 남겨보세요.</p>
      <Link
        href="/board/register"
        className="inline-flex h-8 shrink-0 items-center rounded-lg border border-border px-3 text-sm font-medium hover:bg-muted"
      >
        일반글 쓰기
      </Link>
      <Link
        href="/qna/register"
        className="inline-flex h-8 shrink-0 items-center rounded-lg bg-primary px-3 text-sm font-medium text-primary-foreground hover:bg-primary/90"
      >
        질문하기
      </Link>
    </div>
