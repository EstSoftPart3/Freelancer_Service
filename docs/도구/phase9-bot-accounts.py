# -*- coding: utf-8 -*-
"""
Phase 10 — 봇 계정 100개 생성. 공용 DB, 승인 후 실행.

  드라이런(기본): !python "C:/dev/Freelancer_Service/docs/도구/phase9-bot-accounts.py"
  실제 실행     : !python "C:/dev/Freelancer_Service/docs/도구/phase9-bot-accounts.py" --apply
  롤백          : !python "C:/dev/Freelancer_Service/docs/도구/phase9-bot-accounts.py" --rollback
...
def targets():
    ...
    for user_id, nickname in BOT_NICKNAMES.items():
        ...
        rows.append(dict(
            ...
            # phase7/8 은 f'010000090{n:02d}' 였다. n=100 에서 12자리로 붕괴해
            # 앞자리를 하나 줄였다 — n=1~99 구간은 값이 완전히 같아 호환된다.
            #   n=1   -> 01000009001 (기존과 동일)
            #   n=100 -> 01000009100 (11자리 유지)
            user_phone_num=f'01000009{n:03d}',
...
def main():
    ...
    try:
        ...
        if mode == 'dry-run':
            ...
            print('!python "C:/dev/Freelancer_Service/docs/도구/phase9-bot-accounts.py" --apply')
            return
